...
class Funcoes
{
	public static function iniciais($str)
	{
		$pos = 0;
		$saida = '';
	...
	public static function moneyR($simbolo, $str)
	{
		$retirar = array($simbolo);
		$valor = str_replace($retirar, ".", $str);
		return $valor;
	}
	...
	{
		$periods = array("segundo", "minuto", "hora", "dia", "semana", "mese", "ano", "d�cada");
	...
	public static function fdataTempo($data, $tipo)
	{
		$newDate = explode(' ', $data);
		$exData = explode($tipo, $newDate[0]);
		if ($tipo == '/') {
			$date = $exData[2] . '-' . $exData[1] . '-' . $exData[0];
		} else if ($tipo == '-') {
			$date = $exData[2] . '/' . $exData[1] . '/' . $exData[0];
		...
		return $date . ' ' . '<span class="text-warning">' . $newDate[1] . '</span>';
	}
	...
	public static function showmes($strmes)
	{
		...
		$meses = array('a', 'Janeiro', 'Fevereiro', 'Mar�o', 'Abril ', 'Maio', 'Junho', 'Julho', 'Agosto', 'Setembro', 'Outubro', 'Novembro', 'Dezembro');
		return $meses[$strmes];
	}
	...
	public static function extensoReais($valor = 0, $maiusculas = false)
	{
		...
		$plural = array(
			"centavos", "reais", "mil", "milh&otilde;es", "bilh&otilde;es", "trilh&otilde;es",
			"quatrilh&otilde;es"
		);
		...
		$u = array(
			"", "um", "dois", "tr&ecirc;s", "quatro", "cinco", "seis",
			"sete", "oito", "nove"
		);
	...
	public static function NotificaEmail($emailSend, $typeMail, $transacao, $place)
	{
		...
		// TIPOS = 1 - cadastro | 2 - recupera senha | 3 - nova compra | 4 - altera�ao na compra
		...
		if ($totalUser == 0) { //  NAO ENCONTROU NENHUM USUARIO COM ESTE EMAIL
			return false;
		} else { // USU�RIO ENCONTRADO $user
			// define o tipo do email
			if ($typeMail == 1) { // cadastro
				$subject = "AGT Turismo - Seja bem vindo � AGT";
				$mensagem = '<html>
									...
									<tbody>
									<tr>
									<td colspan="2">Ol�, <b>' . $user['0']['nome'] . '</b>. Seja bem vindo � AGT Turismo.</td>
									</tr>
									<tr>
									<td colspan="2">Em nosso sistema voc� pode comprar passagens rodovi�rias para diversos destinos, com a seguran�a e qualidade AGT Turismo que voc� j� deve conhecer.</td>
									</tr>
									<tr>
			...
			if ($typeMail == 2) { // 2 - recupera senha
				...
				$mensagem = '<html>
									...
									<tbody>
									<tr>
									<td colspan="2">Ol�, <b>' . $user['0']['nome'] . '</b>.</td>
									</tr>
									<tr>
									<td colspan="2">Segue abaixo os sesus dados de acesso:</td>
									</tr>
			...
			if ($transacao != 0) { // S� EXECUTA COM TRANSACAO VALIDA
				...
				if ($totalPed != 0) {
					...
					if ($typeMail == 3) { // 3 - nova compra
						...
						$mensagem = '<html>
													...
													<tbody>
													<tr>
													<td colspan="2">Ol�, <b>' . $user['0']['nome'] . '</b>. Seu pedido de n�mero ' . $ps['0']['id'] . ' foi iniciado e estamos aguardando o pagamento.</td>
													</tr>
													<tr>
													<td colspan="2">Segue abaixo as informa��es do seu pedido:</td>
													</tr>
													...
													<tr>
													<td colspan="2">Se voc� ainda n�o pagou pode acessar � �rea do cliente no site, escolher o pedido, clicar em "Ver detalhes" e depois em "Pagar viagme".</td>
													</tr>

													<tr>
													<td colspan="2">Caso j� tenha efetuado o pagamento aguarde at� que o sistema confirme a transa��o e logo voc� receber� um e-mail com a confirma��o do pagamento.</td>
													</tr>
					...
					if ($typeMail == 4) { // 4 - altera�ao na compra
						...
						$mensagem = '<html>
													...
													<tbody>
													<tr>
													<td colspan="2">Ol�, <b>' . $user['0']['nome'] . '</b>. O status do pedido ' . $ps['0']['id'] . ' foi alterado para ' . $ps['0']['statustxt_pagamento'] . '.</td>
													</tr>
													<tr>
													<td colspan="2">Segue abaixo as informa��es do seu pedido:</td>
													</tr>
					...
					} // FIM DO TIPO 4 - altera�ao na compra
				}
			}
